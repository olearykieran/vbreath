...
export const useBreathAnimation = (meshRef: React.RefObject<THREE.Mesh>) => {
  const { currentLevel, isPlaying, tempo, setIsInhaling } = useBreathStore();
  ...
  useEffect(() => {
    ...
    const timeline = gsap.timeline({
      ...
      onUpdate: () => {
        ...
        const inhaleDuration = level.inhaleDur * (1 / tempo);
        const totalDuration = inhaleDuration + level.exhaleDur * (1 / tempo);
        const inhaleEndProgress = inhaleDuration / totalDuration;
    ...
    timeline.to(meshRef.current.scale, {
      ...
      y: 1.3,
      z: 1.3,
      duration: level.inhaleDur * (1 / tempo),
      ease: "power2.inOut"
    });
    ...
    timeline.to(meshRef.current.scale, {
      ...
      y: 1,
      z: 1,
      duration: level.exhaleDur * (1 / tempo),
      ease: "power2.inOut"
    });
    
    animationRef.current = timeline;
  }, [currentLevel, tempo, meshRef, setIsInhaling]);
